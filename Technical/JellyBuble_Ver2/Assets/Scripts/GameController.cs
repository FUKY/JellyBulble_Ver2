...
public class GameController : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Awake()
    {
        uiController = GameObject.Find("Canvas").GetComponentInChildren<UIController>();
    }
    ...
    public void InstantiateTimeStar()
    {
        ...
        do
        {
            row = Random.Range(0, countRow);
            collumn = Random.Range(0, countCollumn);
        ...
        if (arrGem[row][collumn] != null)
        {
            ...
            {
                gem.timeAdd = true;
                Debug.Log("Time Add = " + uiController.timeAddValue);
        ...
        if (ListDelete.Count >= 3)
        {
            ...
            for (int i = 0; i < ListDelete.Count; i++)
            {
                ...
                if (_gemDacBiet.timeAdd == true)
                {
                    ...
                    Gem _gemStart = start.GetComponent<Gem>();                    
                    start.transform.SetParent(gemContainer);
                    if (_gemStart != null)
                    {                        
